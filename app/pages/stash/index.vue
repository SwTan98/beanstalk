...
const { activeBeans, archivedBeans, isLoading, lowStockBeans, archiveBean } = useBeanstalk()

async function handleArchive(beanId: string) {
  if (!window.confirm('Archive this bean? It will stay in history but no longer be available for new brews.')) {
    return
  }
    ...
    <template v-else>
      <div v-if="lowStockBeans.length" class="surface-card px-5 py-5">
        ...
        <p class="mt-2 text-sm text-espresso-700">
          {{ lowStockBeans.length }} bean{{ lowStockBeans.length === 1 ? '' : 's' }} reached the low-stock threshold.
        </p>
      </div>
      ...
      <section v-if="archivedBeans.length" class="space-y-3">
        <div class="flex items-center justify-between">
          ...
          <p class="text-sm text-espresso-500">
            Still linked to past brews
          </p>
        </div>

        <BeanCard
          v-for="bean in archivedBeans"
          :key="bean.id"
          :bean="bean"
        />
      </section>
