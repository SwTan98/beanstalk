...
- **The scan flow consults `/api/parse-label` on every online scan** and has two modes, switched by the `GEMINI_IMAGE_PARSE` env flag (`usePhotoScan.ts`). Flag off (text mode): OCR runs first, the deterministic parser (`bean-label-parser.ts`) is always computed locally, Gemini polishes the OCR lines and wins per field (`useLabelParse.ts`/`label-merge.ts`). Flag on (image mode): OCR is skipped and the bag photo goes straight to Gemini, whose result is used wholesale; on any failure the flow falls back to OCR + deterministic parse only (no second Gemini call), and a successful image scan still warm-caches the OCR models in the background. In both modes every network failure degrades silently to the deterministic result — scanning must always work fully offline.
